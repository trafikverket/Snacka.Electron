...
const REG_OUTPUT_REGISTERED_APP =
  '\n    Snacka    REG_SZ    Software\\Snacka\\Capabilities\n';
const REG_OUTPUT_CAPABILITY_TEL = '\n    tel    REG_SZ    Snacka.tel\n';
const REG_OUTPUT_PROGID_TEL =
  '\n    (Default)    REG_SZ    "C:\\Program Files\\Snacka\\Snacka.exe" -- "%1"\n';
...
describe('getTelephonyDiagnostics — Windows platform checks', () => {
  ...
  it('windows.progid.tel passes when default value contains Snacka.exe', async () => {
    getExecFileMock().mockResolvedValue({
  ...
  it('windows.progid.tel fails when command does not contain Snacka.exe', async () => {
    getExecFileMock().mockResolvedValue({
      stdout:
  ...
  it('isDefault.tel passes when UserChoice ProgId equals Snacka.tel', async () => {
  ...
  it('isDefault.tel passes when UserChoice is missing but UserChoiceLatest equals Snacka.tel', async () => {
    getExecFileMock().mockImplementation((_cmd: string, args: string[]) => {
      const keyPath: string = args[1] ?? '';
      if (keyPath.endsWith('URLAssociations\\tel\\UserChoice')) {
        return Promise.reject(new Error('not found'));
  ...
  it('isDefault.tel fails when UserChoiceLatest points to another handler even if the protocol class launches Snacka', async () => {
    getExecFileMock().mockImplementation((_cmd: string, args: string[]) => {
      const keyPath: string = args[1] ?? '';
      if (keyPath.endsWith('URLAssociations\\tel\\UserChoice')) {
        return Promise.reject(new Error('not found'));
  ...
  it('isDefault.callto passes when no user choice is set but the protocol class launches Snacka.exe', async () => {
    getExecFileMock().mockImplementation((_cmd: string, args: string[]) => {
      const keyPath: string = args[1] ?? '';
...
describe('getTelephonyDiagnostics — macOS platform checks', () => {
  ...
  it('darwin.handler.tel passes when process.execPath lives inside the handler bundle (packaged)', async () => {
    setExecPath('/Applications/Snacka.app/Contents/MacOS/Snacka');
    (appMock.getApplicationInfoForProtocol as jest.Mock).mockResolvedValue({
      name: 'Snacka',
      path: '/Applications/Snacka.app',
      icon: null,
    });
    ...
    expect(check?.status).toBe('pass');
    expect(check?.details).toContain('Snacka');
  });
...
describe('getTelephonyDiagnostics — Linux platform checks', () => {
  ...
  it('linux.xdg.tel passes when stdout contains "snacka"', async () => {
    getExecFileMock().mockResolvedValue({
      stdout: 'snacka.desktop\n',
      stderr: '',
    });
    ...
    expect(check?.status).toBe('pass');
    expect(check?.details).toBe('snacka.desktop');
  });

  it('linux.xdg.tel passes when the default desktop file launches Snacka', async () => {
    getExecFileMock().mockResolvedValue({
      stdout: 'chat.desktop\n',
      stderr: '',
    });
    getReadFileMock().mockResolvedValue(
      '[Desktop Entry]\nExec=/opt/Snacka/snacka %u\n'
    );
    ...
    const check = result.checks.find((c) => c.id === 'linux.xdg.tel');
    expect(check?.status).toBe('pass');
    expect(check?.details).toContain('Exec=/opt/Snacka/snacka %u');
  });
