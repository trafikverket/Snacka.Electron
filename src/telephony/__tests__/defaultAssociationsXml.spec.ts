...
const xmlPath = join(repoRoot, 'build', 'SnackaDefaultAppAssociations.xml');
...
describe('SnackaDefaultAppAssociations.xml', () => {
  ...
  it('declares the tel association with the Snacka.tel ProgId', () => {
    expect(xml).toMatch(
      /Identifier="tel"[^/>]*ProgId="Snacka\.tel"|ProgId="Snacka\.tel"[^/>]*Identifier="tel"/
    );
  });

  it('declares the callto association with the Snacka.callto ProgId', () => {
    expect(xml).toMatch(
      /Identifier="callto"[^/>]*ProgId="Snacka\.callto"|ProgId="Snacka\.callto"[^/>]*Identifier="callto"/
    );
  });

  it('uses the same ProgIds the NSIS installer registers', () => {
    expect(nsh).toContain('Snacka.tel');
    expect(nsh).toContain('Snacka.callto');
  });
    ...
    expect(config.extraResources).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          from: 'build/SnackaDefaultAppAssociations.xml',
          to: 'SnackaDefaultAppAssociations.xml',
        }),
      ])
    );
  });
