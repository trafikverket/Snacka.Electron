...
const commandLaunchesSnacka = (command: string): boolean => {
  const normalizedCommand = command.toLowerCase();
  return (
    normalizedCommand.includes(process.execPath.toLowerCase()) ||
    /(?:^|[\s"'])snacka(?:\.exe)?(?:$|[\s"'])/i.test(
...
const checkIsDefaultOnWindows = async (
  ...
  const label = `${scheme}: is set to Snacka`;
  const expected = `Snacka.${scheme}`;
  try {
    const progId = await queryWindowsUserChoiceProgId(scheme);
    if (progId === null) {
      const command = await queryWindowsProtocolCommand(scheme);
      if (command !== null && commandLaunchesSnacka(command)) {
        return {
          id,
          label,
          status: 'pass',
          details:
            'Windows has no UserChoice ProgId, but the effective protocol command launches Snacka.',
        };
      }
      ...
      return {
        ...
        status: 'fail',
        details:
          'Windows has not been told which app to use for this link. Open default apps and pick Snacka.',
    ...
    return {
      ...
      details:
        progId === expected
          ? undefined
          : `Currently handled by another app (${progId}). Open default apps to switch to Snacka.`,
      action:
...
const checkWindowsRegisteredApp =
  async (): Promise<TelephonyDiagnosticCheck> => {
    ...
    const label = 'Windows: Snacka is in RegisteredApplications';
    const expected = 'Software\\Snacka\\Capabilities';
    try {
...
const checkWindowsCapability = async (
  ...
  try {
    const value = await queryRegValue(
      `Software\\Snacka\\Capabilities\\URLAssociations`,
      scheme
    );
    if (value === null) {
      return {
...
const checkWindowsProgId = async (
  ...
  const progId = `Snacka.${scheme}`;
  const id = `windows.progid.${scheme}`;
  const label = `Windows: ${progId} ProgID points at Snacka.exe`;
  try {
    const value = await queryRegValue(
    ...
    const passes = commandLaunchesSnacka(value);
    return {
      id,
      label,
...
const checkDarwinHandler = async (
  ...
  const label = `macOS: ${scheme}:// handler reports Snacka`;
  try {
    ...
    // packaged builds (Snacka.app === Snacka.app). The intent is
...
const checkLinuxXdg = async (
  ...
  const label = `Linux: xdg-mime default for ${scheme} is Snacka`;
  try {
    ...
    const desktopIdLooksSnacka = trimmed.toLowerCase().includes('snacka');
    const desktopExec = desktopIdLooksSnacka
      ? null
      : await readLinuxDesktopExec(trimmed);
    const passes =
      desktopIdLooksSnacka ||
      (desktopExec !== null && commandLaunchesSnacka(desktopExec));
    return {
      id,
      label,
      status: passes ? 'pass' : 'fail',
