...
describe('servers/preload window helpers', () => {
  ...
  it('normalizes Snacka title for non-default host', () => {
    ...
    expect(mockedDispatch).toHaveBeenCalledWith({
      ...
      payload: {
        ...
        title: 'Snacka - https://example.com',
      },
    });
  });

  it('keeps Snacka title for the default host', () => {
    mockedGetServerUrl.mockReturnValue('https://snacka.app.trafikverket.se');
    setTitle('Snacka');

    expect(mockedDispatch).toHaveBeenCalledWith({
      type: WEBVIEW_TITLE_CHANGED,
      payload: { url: 'https://snacka.app.trafikverket.se', title: 'Snacka' },
    });
  });
